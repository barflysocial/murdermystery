...
function validateTrainingUnlock(){
  try{
    return localStorage.getItem(TRAINING_UNLOCK_KEY) === 'true';
  }catch(e){
    return false;
  }
}
...
async function init(){
  try{
    if(!validateTrainingUnlock()){
      el.panel.innerHTML = `<div class="start-card"><h2>Access Locked</h2><div class="body">Complete Police Academy to unlock Case 001.</div><div class="footer-actions" style="margin-top:14px"><button class="primary" onclick="window.location.href='${TRAINING_URL}'">Go to Training</button></div></div>`;
      return;
    }
...
function submitFinal(){
  ...
  if(state.unlockState === 'unlock'){
    startUnlockFlow(
      'Case Solved',
      'Promotion Earned: Junior Detective',
