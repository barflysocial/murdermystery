const ROUND_CONFIG = {"firstSceneByRound": {"1": "scene_001_opening", "2": "scene_006_round2_brief", "3": "scene_013_final_accusation"}, "checkpointByRound": {"1": "scene_005_round1_checkpoint", "2": "scene_008_round2_checkpoint"}, "requiredByRound": {"1": ["scene_001_opening", "scene_002_briefing_table", "scene_003_seating_area", "scene_004_access_board"], "2": ["scene_006_round2_brief", "scene_007_log_review"], "3": ["scene_013_final_accusation"]}};
...
const TRAINING_UNLOCK_KEY = 'barfly_training_complete';
const BARFLY_CURRENT_RANK_KEY = 'barfly_current_rank';
const CASE001_URL = '../case001/';
...
function currentRound(){
  ...
  if(state.flags.has('round_2_complete')) return 'Final';
...
async function init(){
  try{
    const res = await fetch('./police_academy_case.json', { cache: 'no-store' });
    if(!res.ok) throw new Error(`Failed to load police_academy_case.json (${res.status})`);
    game = await res.json();
    el.heroTitle.textContent = game.case?.title || 'Police Academy';
    el.heroSub.textContent = `${game.case?.subtitle || ''} Press Start to begin Attempt 1.`.trim();
    el.difficultyLabel.textContent = `Difficulty: ${game.case?.difficulty || 'Training'}`;
    hardReset(false);
    bind();
    renderNav();
    render();
  }catch(err){
    el.panel.innerHTML = `<div class="empty bad">Failed to load Police Academy.<br>${escapeHtml(err.message)}</div>`;
  }
}
...
function renderNav(){
  ...
  el.mainNav.innerHTML = items.map(item => {
    ...
    const isCheckpointReady = item.id === 'checkpoint' && started && checkpointUnlocked(currentRound()) && !checkpointAlreadyAnswered(currentRound()) && currentRound() !== 'Final';
...
function renderHome(){
  return `<div class="start-card"><h2>${escapeHtml(game.case?.title || 'Police Academy')}</h2><div class="body">${escapeHtml(game.case?.setting || '')}\n\nThis is a training case. Press Start to begin Attempt 1. Home will hide once the run begins.</div></div>`;
}
...
function submitCheckpointAnswer(sceneId){
  ...
  if(!state.checkpointResults[sceneId]){
    ...
    const roundNum = scene.round;
    if(typeof roundNum === 'number') state.flags.add(`round_${roundNum}_complete`);
  }
...
function submitFinal(){
  ...
  if(state.unlockState === 'unlock'){
    startUnlockFlow(
      'Case Solved',
      'Promotion Earned: Rookie',
      'Successful solve confirmed. Case 001 will unlock after the hold.'
    );
...
function renderResultOverlay(){
  ...
  overlay.innerHTML = `
    <div class="result-card ${isWin ? 'win winner-pulse' : 'fail'}">
      <div class="result-title">${escapeHtml(state.resultOverlay.title)}</div>
      <div class="result-badge ${isWin ? 'win' : 'fail'}">${escapeHtml(state.resultOverlay.badge)}</div>
      <div class="result-sub">${escapeHtml(state.resultOverlay.body)}</div>
      ${isWin ? `<div class="result-countdown">Next screen in ${state.resultCountdownSec}s</div>` : ''}
      <div class="result-actions">
        ${isWin ? '' : `<button class="primary" onclick="beginAttempt()">Retry Level</button>`}
      </div>
    </div>
  `;
...
function startUnlockFlow(messageTitle, badge, messageBody){
  ...
  resultFlowTimer = setTimeout(() => {
    ...
    resultCountdownTimer = null;
    try{
      localStorage.setItem(TRAINING_UNLOCK_KEY, 'true');
      localStorage.setItem(BARFLY_CURRENT_RANK_KEY, 'Rookie');
    }catch(e){}
    state.resultOverlay = {
      kind: 'unlock',
      title: 'Case 001 Unlocked',
      badge: 'Promotion Earned: Rookie',
      body: 'Training complete. Opening Case 001 now.'
    };
    state.resultCountdownSec = 0;
    render();
    setTimeout(() => {
      window.location.href = CASE001_URL;
    }, 1200);
  }, UNLOCK_HOLD_MS);
}
