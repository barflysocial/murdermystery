...
const state = {
  currentSceneId: null, currentTab: 'home', score: 0, flags: new Set(), evidence: new Set(),
  visitedScenes: new Set(), usedHints: {}, interviewState: {}, checkpointResults: {},
  selectedCheckpointAnswers: {}, finalAnswers: {}, finalResult: null, activeInterviewId: null,
  startedAt: null, penaltiesSec: 0, timerSeconds: 0
};
...
function evidenceList(){ return Array.isArray(game?.evidence) ? game.evidence : Object.values(game?.evidence || {}); }
...
async function init(){
  try{
    ...
    el.heroSub.textContent = game.case?.subtitle || 'Start the investigation to unlock the case file.';
    resetCase(false);
    bind();
    renderNav();
    render();
...
function bind(){
  ...
  el.resetBtn.addEventListener('click', ()=> {
    if(confirm('Reset this investigation? This will erase current progress and restart the timer.')){
      resetCase(true); renderNav(); render();
    }
  });
}

function resetCase(includeUI=true){
  started = false; clearInterval(timerInterval); timerInterval = null;
  state.currentSceneId = game?.case?.entryScene || 'scene_001_opening';
  state.currentTab = 'home'; state.score = 0; state.flags = new Set(); state.evidence = new Set();
  state.visitedScenes = new Set(); state.usedHints = {}; state.interviewState = {};
  state.checkpointResults = {}; state.selectedCheckpointAnswers = {}; state.finalAnswers = {};
  state.finalResult = null; state.activeInterviewId = null; state.startedAt = null; state.penaltiesSec = 0; state.timerSeconds = 0;
  el.hintBox.hidden = true; el.hintBox.textContent = '';
  if(includeUI) updateRunUI();
}

function startCase(){
  if(started) return;
  started = true; state.currentTab = 'scene'; state.startedAt = Date.now(); startTimer(); renderNav();
  goScene(game.case?.entryScene || 'scene_001_opening');
}

function startTimer(){
  clearInterval(timerInterval);
  timerInterval = setInterval(()=>{ if(!started || !state.startedAt) return; state.timerSeconds = Math.floor((Date.now() - state.startedAt)/1000); updateRunUI(); }, 1000);
  updateRunUI();
}
...
function updateRunUI(){
  ...
  if(started && state.timerSeconds >= 120) el.finalTimeCard.classList.add('timer-warn'); else el.finalTimeCard.classList.remove('timer-warn');
}

function renderNav(){
  el.mainNav.innerHTML = NAV_ITEMS.map(item=>{
...
function goScene(id){
  if(typeof id === 'string' && id.startsWith('__INTERVIEW__:')){
    const interviewId = id.split(':')[1]; state.activeInterviewId = interviewId; state.currentTab = 'interviews'; renderNav(); render(); return;
  }
  const scene = sceneMap()[id]; if(!scene) return;
  state.currentSceneId = id; state.visitedScenes.add(id);
  (scene.grantsEvidence || []).forEach(x=>state.evidence.add(x));
  (scene.grants || []).forEach(x=>state.evidence.add(x));
  state.currentTab = 'scene'; renderNav(); render();
}
...
function useHint(){
  ...
  if(!hint) return;
  state.usedHints[round] = true; state.penaltiesSec += PENALTIES.hint;
  el.hintBox.hidden = false; el.hintBox.textContent = hint; updateRunUI();
}
...
function renderHome(){ return `<div class="start-card"><h2>${escapeHtml(game.case?.title || 'Crime 001')}</h2><div class="body">${escapeHtml(game.case?.setting || '')}\n\nPress Start to begin the investigation. The timer starts when you do.</div></div>`; }
function renderInstructions(){ return `<div class="start-card"><h2>How to Play</h2><div class="body">1. Press Start to begin the case.\n2. Read each scene and follow the choices.\n3. Collect clues and question suspects and witnesses.\n4. On checkpoint screens, select one answer, then press Submit Answer.\n5. Wrong answers and hints add time penalties.\n6. At the end, make your final accusation.\n\nFastest correct final time wins.</div></div>`; }

function renderScene(){
  const scene = currentScene(); if(!scene) return `<div class="empty">No scene loaded.</div>`;
  if(scene.type === 'checkpoint') return renderCheckpoint(scene);
  if(scene.id === game.case?.finalScene) return renderFinalAccusation();
  let html = `<div class="scene-block"><h2>${escapeHtml(scene.title || '')}</h2>${scene.text ? `<div class="body">${escapeHtml(scene.text)}</div>` : ''}</div>`;
  if(scene.choices?.length){
    html += `<div class="clue"><div class="question">Choose your next action</div><div class="choices">`;
    for(const choice of scene.choices){ const [label, next] = choice; html += `<button class="choice" onclick="goScene('${escapeJs(next)}')">${escapeHtml(label)}</button>`; }
    html += `</div></div>`;
  }
  return html;
}
...
function renderCheckpoint(scene){
  ...
  scene.answers.forEach((ans, idx) => { const label = Array.isArray(ans) ? ans[0] : ans.label; const active = selected === idx ? ' active' : ''; html += `<button class="answer${active}" onclick="selectCheckpointAnswer('${escapeJs(scene.id)}', ${idx})">${escapeHtml(label)}</button>`; });
  html += `</div>`;
  if(!result){
...
function submitCheckpointAnswer(sceneId){
  ...
  const idx = state.selectedCheckpointAnswers[sceneId]; if(idx === undefined || idx === null) return;
  const ans = scene.answers[idx]; const isCorrect = Array.isArray(ans) ? !!ans[1] : !!ans.is_correct;
  if(!state.checkpointResults[sceneId]){
    if(isCorrect) state.score += 25; else state.penaltiesSec += PENALTIES.wrongCheckpoint;
...
function renderClues(){
  ...
  return items.map(item => `<div class="clue"><h3 class="clue-title">${escapeHtml(item.title)}</h3><div class="body">${escapeHtml(item.description || item.short_card || '')}</div>${item.tags ? `<div class="tags">${item.tags.map(t=>`<span class="tag">${escapeHtml(t)}</span>`).join('')}</div>` : ''}</div>`).join('');
}

function renderSuspects(){
  const suspects = game.suspects || [];
  return `<div class="columns">${suspects.map(s => `<div class="suspect"><h3>${escapeHtml(s.name)}</h3><div class="tiny">${escapeHtml(s.role || '')}</div><div class="body">${escapeHtml(s.public_read || s.publicRead || '')}</div></div>`).join('')}</div>`;
}

function renderTimeline(){
  const items = game.timeline?.public || [];
  return items.map(item => `<div class="timeline-item"><h3>${escapeHtml(item.time || '')}</h3><div class="body">${escapeHtml(item.text || '')}</div></div>`).join('');
}

function topicUnlocked(topic){ return hasFlags(topic.requires_flags || topic.requiresFlags || []) && hasEvidence(topic.requires_evidence || topic.requiresEvidence || []); }
...
function openInterviewTopic(interviewId, topicId){
  ...
  store.activeTopic = topicId; (topic.flags || []).forEach(f=>state.flags.add(f)); (topic.grants || []).forEach(g=>state.evidence.add(g)); render();
}

function renderInterviews(){
  const interviews = interviewMap();
  let html = `<div class="clue"><div class="question">Choose an interview</div><div class="choices">`;
  for(const [id, interview] of Object.entries(interviews)){ html += `<button class="choice" onclick="openInterview('${escapeJs(id)}')">${escapeHtml(interview.name)}</button>`; }
  html += `</div></div>`;
  const currentId = state.activeInterviewId; if(!currentId || !interviews[currentId]) return html;
  const interview = interviews[currentId]; const store = state.interviewState[currentId] || {};
  ...
  if(store.activeTopic){
    ...
    if(topic) html += `<div class="response"><h3>${escapeHtml(topic.label)}</h3><div class="body">${escapeHtml(topic.response || '')}</div></div>`;
  }
  return html;
}

function renderFinalAccusation(){
  const questions = game.case?.finalQuestions || []; let html = `<div class="final-group"><div class="question">Final Accusation</div>`;
  ...
  if(state.finalResult) html += `<div class="response"><div class="body">${escapeHtml(state.finalResult.summary)}</div></div>`;
  html += `</div>`; return html;
}

function setFinalAnswer(id, value){ state.finalAnswers[id] = value; }
function submitFinal(){
  const qs = game.case?.finalQuestions || []; let correctCount = 0; let killerCorrect = false;
  for(const q of qs){
    const ans = state.finalAnswers[q.id]; const ok = ans === q.correct; if(ok) correctCount++;
    else{
  ...
  state.finalResult = { summary: `${tier}\nElapsed: ${formatClock(state.timerSeconds)}\nPenalties: +${formatClock(state.penaltiesSec)}\nFinal Time: ${formatClock(state.timerSeconds + state.penaltiesSec)}` };
  updateRunUI(); render();
}

window.switchTab = switchTab; window.goScene = goScene; window.selectCheckpointAnswer = selectCheckpointAnswer;
window.submitCheckpointAnswer = submitCheckpointAnswer; window.openInterview = openInterview;
window.openInterviewTopic = openInterviewTopic; window.setFinalAnswer = setFinalAnswer; window.submitFinal = submitFinal;
