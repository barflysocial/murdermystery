...
function getNavItems() {
  ...
  if (!Array.isArray(tabs) || !tabs.length) return [...fixed, ...DEFAULT_NAV_ITEMS.filter(x => !['home','instructions'].includes(x.id))];
  const dynamic = tabs.map(tab => {
  ...
  }).filter(x => x.id && !['home','instructions'].includes(x.id));
...
function hydrateState(data) {
  ...
  state.currentTab = data.currentTab || 'home';
...
function goScene(id) {
  if (typeof id === 'string' && id.startsWith('__INTERVIEW__:')) {
    openInterview(id.split(':').slice(1).join(':'));
    return;
  }
...
function renderHome() {
  ...
  const found = state.evidence.size;
  const flags = state.flags.size;
  const progress = Math.min(100, Math.round(((found + flags) / Math.max(1, Object.keys(evidenceMap()).length + 10)) * 100));
  el.panel.innerHTML = `
    <div class="view-shell">
      <div class="scene-card hero-scene">
        <div class="scene-kicker">Case Home</div>
        <h2 class="scene-title">${escapeHtml(c.title || 'Case File')}</h2>
        <div class="body">${escapeHtml(c.subtitle || 'Review the file, read the instructions, and start the investigation when you are ready.')}</div>
        <div class="case-chip-row">
          <span class="case-chip">Save: ${saved ? 'Found' : 'None'}</span>
          <span class="case-chip">Evidence Found: ${found}</span>
          <span class="case-chip">Flags: ${flags}</span>
          <span class="case-chip">Progress: ${progress}%</span>
        </div>
        <div class="footer-actions" style="justify-content:center">
          <button class="primary" onclick="switchTab('scene')">Continue Investigation</button>
          <button class="smallbtn" onclick="switchTab('instructions')">How to Play</button>
          <button class="ghostbtn" onclick="startNewRun()">Start Fresh</button>
        </div>
      </div>
    </div>`;
}
...
function renderInstructions() {
  el.panel.innerHTML = `
    <div class="view-shell">
      <div class="scene-card hero-scene">
        <div class="scene-kicker">Instructions</div>
        <h2 class="scene-title">How to Play</h2>
        <div class="body">Read scenes, inspect clues, question suspects and witnesses, and use the timeline and map to narrow the truth. Each round has a checkpoint question. Select one answer, click <strong>Submit Answer</strong>, read the feedback, then continue.</div>
      </div>
      <div class="columns">
        <div class="scene-card">
          <h3 class="section-title">Core Loop</h3>
          <div class="body">1. Read the current scene.\n2. Open clues, suspects, timeline, interviews, and map as needed.\n3. Use one hint per round if needed.\n4. Submit the checkpoint answer.\n5. Reach the final accusation and lock all four answers.</div>
        </div>
        <div class="scene-card">
          <h3 class="section-title">Run Rules</h3>
          <div class="body">The timer is round-based and starts when you enter a playable round scene. Score increases for correct answers and key deductions. Hints reduce score. Your progress can be saved in this browser and resumed later.</div>
        </div>
      </div>
      <div class="footer-actions" style="justify-content:center">
        <button class="primary" onclick="switchTab('scene')">Go to Scene</button>
      </div>
    </div>`;
}
...
function useHint() {
  ...
  const penalty = Number(game?.ui?.hint_penalty || (currentCaseMeta().id === 'case_002' ? 10 : 8));
