...
function renderHome(){
  return `<div class="start-card"><h2>${escapeHtml(game.case?.title || 'Police Academy')}</h2><div class="body">${escapeHtml(game.case?.setting || '')}\n\nThis is a training case. Press Start to begin Attempt 1. Home will hide once the run begins.</div><div class="hero-image"><img src="./police_academy_home.png" alt="Police Academy training case art"></div></div>`;
}
...
function submitFinal(){
  ...
  if(state.unlockState === 'unlock'){
    startUnlockFlow(
      ...
      'Successful solve confirmed. Next case unlocked.'
    );
...
function startUnlockFlow(messageTitle, badge, messageBody){
  ...
  resultFlowTimer = setTimeout(() => {
    ...
    state.resultOverlay = {
      ...
      body: 'Promotion earned. Return next week for your next file.'
    };
    state.resultCountdownSec = 0;
    render();
  }, UNLOCK_HOLD_MS);
