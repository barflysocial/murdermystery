...
function currentRound() { const s = currentScene(); return s?.round ?? s?.round_id ?? 'Final'; }
...
function getRoundConfig(round) {
  ...
  return rounds.find(r => String(r.number) === String(round) || String(r.id) === String(round) || String(r.round_id) === String(round)) || null;
}
...
function render() {
  ...
  if (timerCard) timerCard.classList.toggle('warning', remaining != null && remaining <= 120);
  if (state.activeHint) {
...
async function init() {
  try {
    ...
    try {
      ...
      el.caseSelect.innerHTML = '<option value="case_001.json">Crime 001 — The Karaoke Killer</option>';
      el.caseSelect.value = 'case_001.json';
    }
    await loadCase('case_001.json', { preferSave: true });
  } catch (err) {
