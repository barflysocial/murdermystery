...
function renderCheckpoint(scene){
  const checkpointKey = state.currentSceneId;
  const selected = state.selectedCheckpointAnswers[checkpointKey];
  const result = state.checkpointResults[checkpointKey];
...
function submitCheckpointAnswer(sceneId){
  ...
  state.checkpointResults[sceneId] = {
    correct: isCorrect,
    text: isCorrect ? (scene.correct || 'Correct.') : (scene.incorrect || 'Not quite.')
  };
  updateRunUI();
  render();
