...
function hydrateState(data) {
  ...
  state.selectedCheckpointAnswers = data.selectedCheckpointAnswers || {};
...
function renderCheckpoint(scene) {
  ...
  if (!result) {
    html += `<div class="footer-actions checkpoint-submit-row">`;
    html += `<button class="primary" ${selected === undefined ? 'disabled' : ''} onclick="submitCheckpointAnswer('${escapeJs(scene.id)}')">Submit Answer</button>`;
    html += `</div>`;
  }
  html += `</div>`;
  ...
  if (result) {
    ...
    const continueId = getCheckpointContinueId(scene);
    if (continueId) {
      html += `<div class="footer-actions"><button class="primary" onclick="goScene('${escapeJs(continueId)}')">Continue</button></div>`;
    }
  }
  html += `</div>`;
  return html;
...
function submitCheckpointAnswer(sceneId) {
  ...
  if (!state.checkpointResults[sceneId]) {
    ...
    setFlags([scene.flag, ...(scene.extraFlags || []), ...(scene.sets_flags || [])].filter(Boolean));
  }
