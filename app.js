const ROUND_CONFIG = {"firstSceneByRound": {"1": "scene_201_opening", "2": "scene_208_ava_intro", "3": "scene_215_priya_intro", "4": "scene_220_raymond_intro", "5": "scene_224_records_reveal"}, "checkpointByRound": {"1": "scene_207_round1_checkpoint", "2": "scene_214_round2_checkpoint", "3": "scene_219_round3_checkpoint", "4": "scene_223_round4_checkpoint", "5": "scene_227_round5_checkpoint"}, "requiredByRound": {"1": ["scene_201_opening", "scene_202_body_reveal", "scene_203_cart_edge", "scene_204_espresso_can", "scene_205_suspect_board_intro"], "2": ["scene_208_ava_intro", "scene_209_darius_intro", "scene_210_vanessa_intro", "scene_211_marcus_intro", "scene_212_elise_intro", "scene_213_trent_intro", "scene_213b_jordan_intro", "scene_214_route_records"], "3": ["scene_215_priya_intro", "scene_216_blind_spot_map", "scene_217_route_synthesis"], "4": ["scene_220_raymond_intro", "scene_221_forsythe_intro", "scene_222_stairwell_trace"], "5": ["scene_224_records_reveal", "scene_225_vanessa_followup", "scene_226_case_synthesis", "scene_228_final_accusation"]}};
...
async function init(){
  try{
    const res = await fetch('./case_002.json', { cache: 'no-store' });
    if(!res.ok) throw new Error(`Failed to load case_002.json (${res.status})`);
    game = await res.json();
    el.heroTitle.textContent = game.case?.title || 'Crime 002';
    if(el.eyebrowLabel) el.eyebrowLabel.textContent = `Crime ${game.case?.crime_number || '002'}`;
    if(el.difficultyLabel) el.difficultyLabel.textContent = `Difficulty: ${game.case?.difficulty || 'Intermediate'}`;
    el.heroSub.textContent = (game.case?.subtitle || 'Press Start to begin Attempt 1.') + (game.case?.subtitle ? ' Press Start to begin Attempt 1.' : '');
    hardReset(false);
    bind();
    renderNav();
    render();
  }catch(err){
    el.panel.innerHTML = `<div class="empty bad">Failed to load Crime 002.<br>${escapeHtml(err.message)}</div>`;
  }
}
